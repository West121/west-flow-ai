...
export function AICopilotLauncher() {
  ...
  const isOpen = useAICopilotShellStore((state) => state.open)
  const aiCapabilities = useAuthStore(
    (state) => state.currentUser?.aiCapabilities ?? []
  )
  const isActive = isOpen || routerState.location.pathname.startsWith('/ai')
  ...
  return (
    <Button
      ...
      onClick={() => openCopilot(routerState.location.pathname)}
    >

...
class DynamicBuildAppendRuntimeServiceTest {
    ...
    @Mock
    private DynamicBuildOutcomeRecorder dynamicBuildOutcomeRecorder;
    ...
    @Test
    void shouldPersistSkippedDynamicBuildLinkWhenRuleOnlyStrategyHasNoRuleResult() {
        when(flowableEngineFacade.runtimeService()).thenReturn(runtimeService);
        when(runtimeService.getVariables("instance_1")).thenReturn(Map.of(
                "westflowProcessDefinitionId", "pd_001",
                "westflowProcessKey", "oa_dynamic_append_tasks",
        ...
        ArgumentCaptor<RuntimeAppendLinkRecord> recordCaptor = ArgumentCaptor.forClass(RuntimeAppendLinkRecord.class);
        verify(dynamicBuildOutcomeRecorder).record(recordCaptor.capture());
        RuntimeAppendLinkRecord record = recordCaptor.getValue();
        assertThat(record.appendType()).isEqualTo("TASK");
        assertThat(record.runtimeLinkType()).isEqualTo("DYNAMIC_BUILD_SKIPPED");
        assertThat(record.status()).isEqualTo("SKIPPED");
        assertThat(record.targetTaskId()).isNull();
        assertThat(record.commentText()).contains("未生成附属结构");
    }

    @Test
    void shouldPersistFailedDynamicBuildLinkWhenSubprocessRuleEvaluationFails() {
        when(flowableEngineFacade.runtimeService()).thenReturn(runtimeService);
        when(runtimeService.getVariables("instance_1")).thenReturn(Map.of(
                "westflowProcessDefinitionId", "pd_001",
                "westflowProcessKey", "oa_dynamic_append_subprocess",
        ...
        when(processLinkService.getByChildInstanceId("instance_1")).thenReturn(null);
        when(runtimeAppendLinkService.getByTargetInstanceId("instance_1")).thenReturn(null);
        when(processDefinitionService.getById("pd_001")).thenReturn(buildBrokenRuleParentDefinitionForSubprocess());
        ...
        RuntimeAppendLinkRecord record = recordCaptor.getValue();
        assertThat(record.appendType()).isEqualTo("SUBPROCESS");
        assertThat(record.runtimeLinkType()).isEqualTo("DYNAMIC_BUILD_FAILED");
        assertThat(record.status()).isEqualTo("FAILED");
        assertThat(record.targetInstanceId()).isNull();
        assertThat(record.commentText()).contains("动态构建执行失败");
    }
    ...
    private PublishedProcessDefinition buildBrokenRuleParentDefinitionForSubprocess() {
        return new PublishedProcessDefinition(
                "pd_001",
                "oa_dynamic_append_subprocess",
                "动态附属子流程",
